...
+ 第一行语法更改：第一行含1~2个字符，若为两个字符则第一个字符是#表示升调，b表示降调；第二个字符是大写的A~G或小写的a~g，分别表示大调和小调。若只有一个字符则表示无升降，该字符与有两个字符时第二个字符规则相同。  

+ **特别注意（兼容性提醒）：为了保持一定的向前兼容性和有利于后面的继续开发，整个文件的第一行为下面三种形式开头时表示TMPL3.2以后的文件：**  
  
  - `#`后面加上`a`\~`z`或`A`\~`Z`中任意一个字母

  - `b`后面加上`a`\~`z`或`A`\~`Z`中任意一个字母

  - `a`\~`z`或`A`\~`Z`中任意一个字母

  **除上面三种情况的，均视为是TMPL3.0及以前的文件，TMPL3.2以后对TMPL3.1的语法不提供额外支持**
